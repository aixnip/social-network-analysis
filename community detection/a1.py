...
def bfs(graph, root, max_depth):
    ...
    node_queue = deque()
    node_queue.append((root, 0, 1))
    grey_set = set()
    white_set = set([node for node in graph.nodes()])
    ...
    while(len(node_queue) > 0):
        ...
        if current[1] >= max_depth :
            break
        grey_set.remove(current[0])
        neighbors = graph[current[0]]
                
        for x in neighbors.keys():
            if x not in white_set:
                if node2distances[x] == (current[1]+1):
                    node2num_paths[x] += 1
                    node2parents[x].append(current[0])
            else:
                node2parents[x].append(current[0])
                grey_set.add(x)
                white_set.remove(x)
                node_queue.append((x, current[1] +1, 1))
                node2distances[x] = current[1] +1
                node2num_paths[x] = 1
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    sortedbydistance = sorted(node2distances.items(), key=lambda item: -item[1])
    node_credit = {}
    result = {}
    for node in sortedbydistance:
        node_credit[node[0]] = 1 
    for node in sortedbydistance:
        edge_credit = node_credit[node[0]] / node2num_paths[node[0]]
        parents = node2parents[node[0]]
        for p in parents:
            node_credit[p] += edge_credit
            result[(tuple(sorted([node[0], p])))] = edge_credit
    return result
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in nodes:
        ...
        scores = bottom_up(n, node2distances, node2num_paths, node2parents)
        for edge in scores.items():
            if edge[0] in aprox_betweenness:
                aprox_betweenness[edge[0]] += edge[1]
            else:
                aprox_betweenness[edge[0]] = edge[1]
    for edge in aprox_betweenness.keys():
        aprox_betweenness[edge] /= 2
    return aprox_betweenness
...
def partition_girvan_newman(graph, max_depth):
    ...
    sorted_betweenness = sorted(approximate_betweenness(new_graph, max_depth).items(), key=lambda item: (-item[1], item[0]))
    i = 0
    while len(list(nx.connected_component_subgraphs(new_graph))) < 2 and i < len(sorted_betweenness):
        edge = sorted_betweenness[i][0]
        new_graph.remove_edge(edge[0], edge[1])
        i += 1
    return list(nx.connected_component_subgraphs(new_graph))
...
def norm_cut(S, T, graph):
    ...
    # my own test
    >>> print("%.4f" % round(norm_cut(['A', 'B', 'C'], ['D', 'E', 'F', 'G'], example_graph()),4))
    0.4167
    """
    cut_set = cut(S, T, graph)
    S_vol = volume(S, graph)
    T_vol = volume(T, graph)
    ncv = cut_set / S_vol + cut_set / T_vol
    return ncv
...
def score_max_depths(graph, max_depths):
    ...
    result = []
    for d in max_depths:
        new_graph = partition_girvan_newman(graph ,d)
        S_nodes = new_graph[0].nodes()
        T_nodes = new_graph[1].nodes()
        ncv = norm_cut(S_nodes, T_nodes, graph)
        result.append((d, ncv))
    return result
...
def make_training_graph(graph, test_node, n):
    ...
    """
    new_graph = graph.copy()
    sorted_neighbors = sorted(new_graph.neighbors(test_node))
    i = 0
    while i < n:
        new_graph.remove_edge(sorted_neighbors[i], test_node)
        i += 1
    return new_graph
...
def jaccard(graph, node, k):
    ...
    [(('D', 'E'), 0.5), (('D', 'A'), 0.0)]
    """
    existing_neighbors = set(graph.neighbors(node))
    scores = []
    for n in graph.nodes():
        if n not in existing_neighbors and n != node:
            neighbors2 = set(graph.neighbors(n))
            scores.append(((node, n), 1. * len(existing_neighbors & neighbors2) / len(existing_neighbors | neighbors2)))
...
def evaluate(predicted_edges, graph):
    ...
    existing = 0
    predict_set = set(predicted_edges)
    return len(edges_set & predict_set) / len(predict_set)
