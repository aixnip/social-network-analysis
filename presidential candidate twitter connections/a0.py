...
def read_screen_names(filename):
    ...
    with open(filename, 'r') as file:
        filelines_list = file.read().split('\n')[:-1]
    return filelines_list
...
def get_users(twitter, screen_names):
    ...
    [6253282, 783214]
    """
    sn_param = ",".join(str(x) for x in screen_names)
    req = robust_request(twitter, "users/lookup", {"screen_name":sn_param})
    return list(req)
...
def get_friends(twitter, screen_name):
    ...
    """
    req = robust_request(twitter, "friends/ids", {"screen_name":screen_name})
...
def count_friends(users):
    ...
    for user in users:
        totalCounter += Counter(user['friends'])
    return totalCounter
...
def friend_overlap(users):
    ...
    for i in range (0, len(users)-1):
        for j in range (i+1, len(users)):
            overlap = len([x for x in users[i]['friends'] if x in users[j]['friends']])
            pairs.append((users[i]['screen_name'], users[j]['screen_name'], overlap))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    #select Hilary and Trump
    hilary = next((user for user in users if user['screen_name'] == 'HillaryClinton'), None)
    trump = next((user for user in users if user['screen_name'] == 'realDonaldTrump'), None)
    #find overlap
    overlap_id = [x for x in hilary['friends'] if x in trump['friends']]
    #requiry api for screen name by id
    req = robust_request(twitter, "users/lookup", {"user_id":overlap_id})
    result = next(user for user in req)
...
def create_graph(users, friend_counts):
    ...
    #find friends followed by more than one candidate
    friends = list(filter(lambda friend: friend[1] > 1 ,friend_counts.most_common()))
    friend_ids = {friend[0] for friend in friends} 
    graph.add_nodes_from([friend[0] for friend in friends])
    #add edges
    for user in users:
        for cand_friends in user['friends']:
            if cand_friends in friend_ids:
                graph.add_edge(user['id'], cand_friends)
    return graph
...
def draw_network(graph, users, filename):
    ...
    labels = {}
    for user in users:
        labels[user['id']] = user['screen_name']
    nx.draw(graph,labels=labels,node_color=range(num_nodes),node_size=40,cmap=plt.cm.summer, linewidths=0.25, width=0.25, edge_color='#cccccc', font_size=10)
